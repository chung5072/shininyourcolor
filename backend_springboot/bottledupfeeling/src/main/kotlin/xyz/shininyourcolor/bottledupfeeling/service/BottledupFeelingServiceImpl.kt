...
import xyz.shininyourcolor.bottledupfeeling.db.repository.BFPCRepository
import xyz.shininyourcolor.bottledupfeeling.dto.request.TalkContent
import xyz.shininyourcolor.bottledupfeeling.dto.request.UncategorizedTalk
import xyz.shininyourcolor.bottledupfeeling.dto.response.TalkCategory

@Service("bottledupFeelingService")
class BottledupFeelingServiceImpl(
    val webClient: WebClient,
    val bfpcRepository: BFPCRepository
) : BottledupFeelingService {
    /**
     * 유저가 입력한 문장을 분류
     *
    ...
    private fun saveCategorizedTalk(
        ...
        val bottledupFeelingPageContents = BottledupFeelingPageContents(
            uuid = uncategorizedTalk.uuid,
            typeDate = uncategorizedTalk.time,
            typeContent = uncategorizedTalk.content
        )
        bottledupFeelingPageContents.classificationId = when (category) {
            "none" -> 1
            "compare" -> 2
            "offensive" -> 3
            "specific_praise" -> 4
        ...
        bfpcRepository.save(bottledupFeelingPageContents)
    }
}